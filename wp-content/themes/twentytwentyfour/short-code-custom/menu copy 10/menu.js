jQuery(document).ready(function ($) {
    ...
    moveHeaderLinksToMobile();
     ...
     // Only click the icon toggles the menu
    $('.vnexpress-hamburger .hamburger-icon').on('click', function(e) {
        e.stopPropagation();
    ...
    $(document).on('click', function(e) {
        if (!$(e.target).closest('.vnexpress-menu-wrapper, .hamburger-icon').length) {
            $('.vnexpress-menu-wrapper').slideUp(300).removeClass('open');
        }
    });
